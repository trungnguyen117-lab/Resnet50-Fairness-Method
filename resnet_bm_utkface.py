...
class FCResNet50(nn.Module):
    def __init__(self, num_classes=2):
        super(FCResNet50, self).__init__()
        ...
        # Add new fully connected layer for our task
...
def set_model():
    model = FCResNet50(num_classes=2).cuda()
    pred = nn.Linear(2048, 2).cuda()  # Using 2048 features from ResNet50

    models = {"model": model, "pred": pred}
    criterion = {"bin": nn.BCEWithLogitsLoss(reduction="none"), 
...
def main():
    opt = get_args()
    exp_name = f"bm_{opt.mode}-utkface_{opt.sensitive}-lr{opt.lr}-bs{opt.batch_size}-epochs{opt.epochs}-seed{opt.seed}-resnet50"
    ...
    train_loader = get_utk_face(root, batch_size=opt.batch_size, bias_attr=opt.sensitive, 
                               split="train", aug=False, sampling="bin")
