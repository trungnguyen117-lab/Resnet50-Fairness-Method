...
from models import Discriminator, Generator, ResNet50
...
class FAAP:
    ...
    def train_batch(self, images, labels, biases):
        ...
        logits_bias = self.netD(features.view(features.size(0), -1).detach())
...
if __name__ == "__main__":
    ...
    ckpt_path = f"{log_name_d}/best_model_50.pt"
    print(ckpt_path)
    deployed_model = ResNet50(num_classes=num_classes, pretrained=False).cuda()
    ...
    final_model_path = Path(f"{log_name_g}/final_model.pt")
    torch.save(faap.netG.state_dict(), final_model_path)
